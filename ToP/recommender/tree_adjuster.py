...
class TreeNode:
    ...
    def remove_sub_branch(self, sub_node:"TreeNode"):
        assert sub_node in self.sub_branches, "sub_node not in sub_branches"
    ...
    def get_leaves(self) -> list["TreeNode"]:
        ...
        if self.is_leaf():
            ...
            return [leaf for sub_branch in self.sub_branches for leaf in sub_branch.get_leavs()]
...
class TreeManager:
    ...
    async def rebalance_tree(self, 
        ...
        while queue and operation_count < max_operations:
            ...
            if current.is_leaf() and current not in processed:
                ...
                if load == 0:
                    self.prune_empty_nodes(current)
    ...
    def get_tweets(self, id_list:list[int], sample_size = None, return_df:bool = False) -> list[str]:
        ...
        '''
        sub_df = self.tweet_df[self.tweet_df['leaf_id'].isin(id_list)]
        if sample_size:
    ...
    async def tweets_reassign(self, node1:TreeNode, node2:TreeNode, mids: list[int]):
        ...
        async def tweet_reassign(tweet_content:str, mid:int) -> None:
            ...
            """
            reassign_prompt = self.prompts["tweets_reassign"].format(
        ...
        tasks = []
        tweet_df = self.get_tweets(ids=mids, return_df=True)
        for mid in mids:
...
def _build_subtree(node_dict: dict, df: pd.DataFrame) -> TreeNode:
    ...
    node = TreeNode(
        name=node_dict["name"],
        criteria=node_dict.get("criteria")
    )
...
def tree_to_dict(root: TreeNode) -> dict:
    ...
    result = _traverse(root)
    result["root"] = root.name  
    return result

if __name__ == "__main__":
    root_path = './../../dataset/twitter'
    tree_path = root_path + 'kmeans_category_trees.json'
    tweet_path = root_path + 'kmeans_tree_classified_root_content_test.csv'
    with open(tree_path, 'r', encoding='utf-8') as f:
        trees = json.load(f)
    tree = trees[0]
    category = tree["root"]
    tweet_df = pd.read_csv(tweet_path)
    sub_df = tweet_df[tweet_df["category"] == category]
    root = dict_to_tree(tree_dict=tree, df = sub_df)
    manager = TreeManager(tree = root, tweet_df = sub_df, model_name= "Qwen2.5-32B-Instruct-AWQ")
    asyncio.run(manager.rebalance_tree())